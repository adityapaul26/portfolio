...
export function Terminal({
  ...
  }, [phase, currentCommand, currentOutputs.length, isLastCommand, onDone]);
  ...
  return (
    ...
    >
      <div className="overflow-hidden rounded-lg border border-neutral-800 bg-neutral-900 shadow-2xl">
        ...
        <div
          ref={contentRef}
          className="no-visible-scrollbar h-80 overflow-y-auto p-4 font-mono relative"
        >
          {!hasStarted && (
            <div className="absolute inset-0 z-10 flex items-center justify-center bg-neutral-900/50 backdrop-blur-[2px]">
              <button
                onClick={handleStart}
                className="bg-white text-black px-6 py-2 rounded-sm font-bold text-xs uppercase tracking-widest hover:bg-purple-400 transition-colors shadow-xl active:scale-95"
              >
                Enter Terminal
              </button>
            </div>
